...
:With ⎕NS⍬ ⋄  DEBUG NOCM←0 0 
  ⎕IO ⎕ML←0 1
  nlC← ⎕UCS 13
  qtC← ''''
  optsModeM← ⍠('UCP' 1)('Mode' 'M')('EOL' 'LF')
  ...
  ⍝ Major functions
    ...
    ⍝ Command parser (PCRE-based)
      ...
      ⍝ Regex Patterns
       ...
       ⍝ Continuations: 2-3 dots OR ellipses Unicode char. Special: ➤, →→ - see Pipes
        continueP← '\h*(?:\.{2,3}|…)\h*((?:⍝.*)?)\R' 
        continueG← ⍬
        pats← qtsP cmP continueP '\R?$' 
              qtsI cmI continueI  nlI← ⍳≢pats
        Continuations← pats ⎕R {  
            ∆F← ⍵.{Lengths[⍵]↑Offsets[⍵]↓Block} ⋄ C←  ⍵.PatternNum∘∊
            C qtsI cmI:  ∆F 0 
            C continueI: ' ' ⊣ continueG,← ⊂∆F 1
            C nlI:       (continueG⊢← ⍬)⊢ (∊' ',¨continueG/⍨0≠≢continueG), nlC
        } optsModeM
    :Section Pipes
      ...
      ⍝       ⋄ bigRA← '➤'   ⍝ 1048...
      ⍝       pats← skipP pipeEndP '\n?$'
      ⍝             skipI cmI pipeEndI  nlI← ≢⍳pats
      ⍝       lines← pats ⎕R {  
      ⍝           ∆F← ⍵.{Lengths[⍵]↑Offsets[⍵]↓Block} ⋄ C←  ⍵.PatternNum∘∊
      ⍝         C skipI: ∆F 0  
      ⍝         C pipeEndI:   ' ',bigRA,' '⊣ continueG,← ⊂∆F 1 
      ⍝         C nlI:   (continueG⊢← ⍬)⊢ (∊' ',¨continueG/⍨0≠≢continueG), ∆F 0
      ⍝       } optsModeM ⊆⍵
      ⍝       ⋄ pipeAnyP← '(?:➤|→→)' ⋄ cmStr←''   
      ⍝       ⋄ pats← qtsP cmP pipeAnyP,¨ ⊂'\h*'    ⍝ Here comments are not simply ignored...
      ⍝       ⋄ qtsI cmI pipeAnyI← ⍳≢pats
      ⍝       lines← { 
      ⍝         '➤→' (∧/~⍤∊) ⍵: ⍵ 
      ⍝           sects← TrimR¨ pats ⎕R {   
      ⍝                 f0← ⍵.Match ⋄ C← ⍵.PatternNum∘∊
      ⍝               C qtsI: f0 
      ⍝               C cmI: ''⊣ cmStr,← f0, ' '/⍨ 0≠ ≢cmStr 
      ⍝                 nlC
      ⍝           }¨ lines
    ...
    :EndSection Pipes
        ...
        pats←   qtsP cmP nmP defP evalP undefP copyP includeP ifCondP endIfP showP unknDirP   
                qtsI cmI nmI defI evalI undefI copyI includeI ifCondI endIfI showI unknDirI← ⍳≢pats
        ProcLns← ⍬∘{
           Match← pats ⎕R { 
            ⍝ extern: linesG
              ∆F← ⍵.{Lengths[⍵]↑Offsets[⍵]↓Block} ⋄ C←  ⍵.PatternNum∘∊
    ...
    :EndSection Command Parser
        ...
        lnsOut← (1↑myLns), ProcLns Continuations 1↓myLns
...
⍝H
⍝H Directives (case is ignored on directive names; "␢" represents zero or more space chars.
...
⍝H                             Note: "␢" represents zero or more space characters.
