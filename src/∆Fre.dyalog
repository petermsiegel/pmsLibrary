...
⍝ Constants 
  ...
  ⍝ Patterns 
    cfpp← '\$\$' '\$' '%' '(?:`⍵|⍹)(\d*)' '(?:"[^"]*")+|(?:''[^'']*'')+'
    ⍝ markCFp matches recursively balanced braces {}, skipping quotes "..." ''...'' and escapes `.  
    markCFp← '(?x) (?<P> (?<!`) \{ ((?>  [^{}"''`]+ | (?:`.)+ | (?:"[^"]*")+ | (?:''[^'']*'')+ | (?&P)* )+)  \} )' 
  ⍝ Errors 
...
⍝ Functions
  ⍝ TF: We pass namespace ¨extern¨ as left arg ⍺.
    TF← { spQ, qSp,⍨ escDmd escEsc escLb escRb q ⎕R ⍺.cr esc lb rb qq _Opts ⍵ }
  ⍝ Omega: We pass namespace ¨extern¨ as left arg ⍺.
    Omega← { 0=≢⍵: ⍕⍺.(omIx← omIx+1) ⋄ ⍕⍺.omIx← ⊃⌽r }
  ...
  ⍝ DocCheck: Checks for document strings,
  ...
  ⍝ CF:  
   ⍝ * We pass namespace ¨extern¨ as left arg ⍺.
    CF← { 
        extern←⍺ 
        isSF sfCod← extern SFCheck ⍵                          ⍝ Space field? 
      isSF: sfCod
  ...
  ⍝ ∘ "Older" style (execute fields R-to-L, display L-to-R): Order← ∊'⍬',⍨⊢
    OrderFlds← '⌽'∘,⍤ ∊'⍬'∘,⍤ ⌽
    MarkCF← markCFp ⎕R '\n\1\n' _Opts
  ⍝ ProcFlds: We pass namespace ¨extern¨ as left arg ⍺.
    ProcFlds← { 0=≢⍵: '' ⋄ lb=⊃⍵: ⍺ CF 1↓¯1↓⍵ ⋄ ⍺ TF ⍵ } 
    FSConvert← { 
        (dfn dbg box) fStr← ⍺ ⍵  
        (extern←⎕NS⍬).(omIx dbg cr)← 0 dbg (dbg⊃ cr crVis)         ⍝ crVis: '␍'
        fmtAll← box⊃ cMerge cDisplay 
        flds← OrderFlds extern∘ProcFlds¨ MarkCF ⊂fStr 
