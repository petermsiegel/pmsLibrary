...
∇ ∆FⓇ← {∆FⓄ} ∆Fc ∆FⒻ ; ⎕TRAP 
  ...
  ##.∆F← ∆Fc 
      ...
      ⍝ ∆F[4|2]:    rc [outStr|outDfn]← ∇ (escCh, options) fString outLen outLen
        '∆F4' ⎕NA 'I4 ∆Fc/∆F.dylib|fs_format4 <{C4 U1[5]} <#C4[] >#C4[] I4'  ⍝ #C4: Up to 2≠64 items 
        '∆F2' ⎕NA 'I4 ∆Fc/∆F.dylib|fs_format2 <{C4 U1[5]} <#C2[] >#C2[] I4'  ⍝ #C2: Up to 2*32 items
      ⍝ Canon: canonStr← ∇ inStr escCh
        'Canon' ⎕NA '∆Fc/∆F.dylib|canon =#C4[] C4'       
      ...
      ⍝ See FStr2Code below.
        Call∆Fc← { isW4 gF← ⍵⍵ 
          ...
          ⍺≤0:      res2, ⍵                      ⍝ If we've tried too many times, return (with error code) as is.
            newSize← 1024⌈ gF× ⍵                 ⍝ Increase the storage estimate and retry...
      ...
      ⍝    result← opts ∇ fstring
        FStr2Code← {    
          ...
          ⍝ BufSize: Initial estimate of max # of (2- or 4-byte) chars needed in output string.
            escCh dfn debug box useNs lib bufSize← EvalOpts ⍺
          ...
          ¯1≠ rc:  rc  ⎕SIGNAL⍨ (⎕EM rc),': ', res              ⍝ Failure w/ error msg passed from ∆F4/2.  
            ErrorSpace bufActual                         ⍝ Failure ¯1: Not enough space for output buffer.
        }
      ⍝ ErrorSpace: output buffer too small to store/evaluate executable generated from fstring.
