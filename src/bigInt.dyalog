...
⍝ --------------------------------------------------------------------------------------------------
  ...
  ⍝ bi:  Returns ⎕THIS
   ...
   ⍝¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯⍝
        ∇ {r}←_LoadPats;actArgScalQ;actArgVecQ;actBiCallNoQ;actBiCallQ;actKeep;lD;lM;p2Fancy;p2Funs1;p2Funs2;p2Ints;p2Plain;p2Vars;pAplInt;pBI;pCom;pFancy;pFunsBig;pFunsNoQ;pFunsQ;pFunsSmall;pNonBiCode;pQot;pVar;t1;t2;tD1;tDM;tM1;tMM
   ...
   ⍝  Distinguish Cases:          Action               Compile-time  Run-time (import)
   ⍝     [1] short ints (<RX)     Keep as APL ints        12345      Fast conversion
   ⍝     [2] longer ints(≥RX)     Quote                  '123…999'   Fast conversion
   ⍝     [3] ints with exponents  Keep as APL int*        1E723      Expand to '1000…000' ≢⍵ = 724
   ⍝         (*) to avoid compiling as large string.
   ⍝
          p2Fancy←'(?:[-¯]?[\d_]+)(?i:[¯.JE]+[\d_]+)+'  ⍝ numbers w/ exponents, decimals, and complex numbers
          p2Plain←'(?i:[-¯]?\d{1,',(⍕⎕PP),'}((?![\d.EJ])|$))' ⍝ Numbers that fit within 1..⎕PP are safe
   ⍝ …P:  Patterns. Most have a field#1
          ...
          pFancy←'(',p2Fancy,')'                   ⍝ See NOTE 20181006 below
          pAplInt←'(',p2Plain,'|',p2Fancy,')'      ⍝ Single (APL) integer values and values with Exponents
                                                   ⍝ are not quoted. Faster.
          pBI←'([-¯]?[\d_]+)'                      ⍝ Put other valid BI-format integers in quotes
   ⍝¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯⍝
   ⍝ BI Actions
   ⍝ …AB: BI (Big Integer) action building-blocks
          actBiCallNoQ←'(\1',(⍕⎕THIS),'.BIX)'       ⍝ See pFunsNoQ above
          actBiCallQ←'(''\1''',(⍕⎕THIS),'.BIX)'     ⍝ See pFunsQ above
          actKeep actArgScalQ actArgVecQ←'\1' '(,''\1'')' '''\1'''
   ...
   ⍝ pAplInt replaced by pFancy-- see NOTE 20181016 below
      ⍝ WAS: pBiCalls←pCom pFunsQ pVar pQot pFunsNoQ pAplInt pNonBiCode pBI
      ⍝ TO:
          pBiCalls←pCom pFunsQ pVar pQot pFunsNoQ pFancy pNonBiCode pBI
   ⍝ EXTERN actBiCalls:   BI (Big Integer) action
      ⍝ NOTE 20181016
      ⍝ WAS: This allows APL-style integers to not be quoted.
      ⍝      This messes up bi.dc (desk calculator) with small integers with underscores.
      ⍝   actBiCalls←actKeep actBiCallQ actKeep actKeep actBiCallNoQ actKeep actKeep actArgVecQ
      ⍝ TO:  quote all numbers except those with exponents...
          actBiCalls←actKeep actBiCallQ actKeep actKeep actBiCallNoQ actKeep actKeep actArgVecQ
