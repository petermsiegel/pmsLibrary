...
⍝     Returns one or more vectors of vectors... (Use ⊃res if one line expected/required).
  Scan4Special←{⍺←0
    ...
    ⍝ pHere:       :::     1=end_token             2=opts    3=doc    4=indent :  \1                 :  5=trailing...
      ...
      iTrpQ iDblQ iSkip iHere iPtr←0 1 (2 3) 4 5
    ⍝ 0 Triple 1 Double 2 Skip  3 Skip    4 Here
        pTriple  pDouble  pSingle pComments pHere  pPtr ⎕R{  
          ...
          CASE iPtr:   '(',(⊃Scan4Special F 1),' ⎕SE.∆PTR 0)'⊣fSave∆PTR∘←1
  ...
  Save∆PTR←{0:: ⎕SIGNAL/'∆FIX: Can''t set ∆PTR' 11
    save←{4=nc←⎕SE.⎕NC ⍵: 0 ⋄ 0≠nc: ∘∘∘ ⋄ ⎕SE.∆PTR←{ns←⎕NS '' ⋄ ns.∆FN←⍺⍺ ⋄ ns} ⋄ 0 }
    fSave∆PTR: ⍵ ⊣ save'∆PTR' ⋄ ⍵ 
  }
  ⍺←⊢ ⋄ fSave∆PTR←0
  Save∆PTR ⍺(⊃⎕RSI).⎕FIX Scan4Special LoadLines ⍵  
}
