∆FIX←{⎕IO ⎕ML←0 1
  ...
  ⍝ Return APL single-quoted equivalents, converted via Flatten above.
    Scan4Special←{⍺←0
      ...
      ⍝ useVV:   ⍺=1:  return vec of strings;   ⍺=0: (default) return single string w/ CR-separated lines
      ⍝ useTrim: ⍺⍺=1: trim each line; else don't.
        Flatten←{
            ⍺←0 ⋄ useVV useTrim←⍺ ⍺⍺
            TrimL←{0=≢⍵:⍵ ⋄ ⍵↓⍨¨lb⌊⌊/lb←+/∧\[1]' '=↑⍵}
      ...
      ⍝ 0Triple 1Double 2Skip   3Skip     4Here
        pTriple pDouble pSingle pComments pHere ⎕R{
          ...
          ⍝ iHere: F 1: VAR_NAME, F 2: options, F 3: END_TOKEN
            CASE iHere:(F 1){                    ⍝ Here-string - :trim (default) vs :notrim
                v nt←1∊¨':vec' ':notrim'⍷¨⊂⍵     ⍝             - :cr separated  lines (default) vs :vec of strings
  ...
  ⍝ Following ⎕FIX syntax, a single line must be a file reference.
    LoadLines←{1<|≡⍵: ⍵ ⋄ p≡⍵↑⍨n←≢p←'file://': ⊃⎕NGET(n↓⍵)1 ⋄ ⎕FIX '∘err'}
