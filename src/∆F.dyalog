...
⍝ ⓁⒻⓉ:  mode debug escCh extLib forceUpdt
...
⍝       escCh ('`'): the "escape" char. If a number is passed, rather than a char,
...
⍝      forceUpdt (0):
⍝          ∘ For debugging only. Forces the external library (see extLib) to be reinstalled;
⍝            and forces the runtime C-based dfn to be reinstalled. 
⍝          ∘ Normally, these actionstake place on the first call to ∆F, w/o user action.
  ...
  ⓄⓊⓉ← (⊃⎕RSI)⍎ ⓁⒻⓉ {     ⍝ The name ⓇⒼⓉ is required for executing (⍎) the result.
      ⎕IO ⎕ML←0 1  
    ⍝ options for ∆F
    ⍝      <= Basic Opts =>  <==  Advanced Opts ==>
    ⍝      mode debug escCh  useNs extLib forceUpdt
      def← 1    0     '`'    0     1      0
      ⍺← def 
      opts← (≢def)↑⍺, def↑⍨ (-≢def)+ ≢⍺
      (o/ opts)← def/⍨ o← opts=¯1
      mode debug escCh useNs extLib forceUpdt← opts  
      escCh← escCh '`'⊃⍨ 0=⊃escCh 
  ...
  ⍝    If libFlag, define ∆F utilities in <lib>.
    LoadLib← forceUpdt ⎕SE.{
      ⍵=0: ⍬ ⋄ (9=⎕NC ⍺)∧ ~⍺⍺: ⍬    ⍝ Don't load if ⍵(extLib)=0 or if ~forceUpdt.
        lib← ⍎⍺ ⎕NS ⍬               ⍝ Load library <⍺>
  ...
  ⍝              )
  ⍝ opts:    mode (1 0 ¯1 ¯2), debug (1|0), useNs (0), extLib (1|0)
  ⍝ escCh:   By default '`' 
