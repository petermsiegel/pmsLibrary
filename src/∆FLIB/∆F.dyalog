  ...
  ∆FⓄ← ∆FⓄ {    
    ...
    ⍝   outPLen: the actual length (in 4-byte chars) of actual code output.
      _←  ⎕SE.⍙F.{  
            _← (C2 C4).⎕EX⍣⍵⊢ '∆FC'
          0(~∊)(C2 C4).⎕NC '∆FC': ⍬
            ⎕← '∆FC' C2.⎕NA⍣(⊃0=C2.⎕NC '∆FC')⊢ 'I4 ∆FLIB/∆F.dylib|fs_format2  <I1[4] C4    <C2[] I4    >C2[]   =I4' 
            ⎕← '∆FC' C4.⎕NA⍣(⊃0=C4.⎕NC '∆FC')⊢ 'I4 ∆FLIB/∆F.dylib|fs_format4  <I1[4] C4    <C4[] I4    >C4[]   =I4' 
            ⍬
      } force                    ⍝  rc                     opts   escCh fStr  ≢fStr res   lenRes
      DOut← { ⍺←'' ⋄ debug=1: ⍵⊣ ⎕←⍺, ⍵ ⋄ ⍵}

    ⍝ Call the C Library!
      lib← (⎕SE.⍙F.(C2 C4)⊃⍨ 323=⎕DR fStr← ⊃⍵).∆FC  
      (rc res lenRes)maxOut ← ( lib { 
            ⍺← 256⌈ (extLib⊃ 128 32) + ((mode=0)+ extLib⊃5 2)× ≢fStr 
            _← ⍺⍺ (mode debug useNs extLib) escCh ⍺⍺ (≢⍺⍺) ⍺ ⍺ 
        (¯1≠ ⊃_)∨ 0≥ ⍵: _ ⍺  
