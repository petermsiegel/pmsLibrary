/* fc4/fc2: Uses 4/2-byte (32-bit) unicode chars throughout   
...
#  include <stdlib.h>  // for alloca
   ...
   #undef afterBlanks 
   #define CHARW CHAR4
   #define afterBlanks afterBlanks4 
#else 
   #undef CHARW 
   #undef afterBlanks  
   #define CHARW CHAR2
   #define afterBlanks afterBlanks2 
#endif 
...
#define OutStr(str)         OutBuf(str, Str2Len(str))
...
#define CodeStr(str)         ADDBUF_GENERIC(str, Str2Len(str), code, 0)  
...
#if USE_ALLOCA
  #define RETURN(rc) *outPLen = outLen; return(rc)
#else
  #define RETURN(rc) if (codeBuf) free(codeBuf); codeBuf = NULL; *outPLen = outLen; return(rc)
#endif
...
#define Scan4OmegaIx(oIx)\
  ...
  int fs_format2(
#endif 
              const char opts[4], const CHAR4 escCh, 
              CHARW fString[],    INT4 fStringLen, 
              CHARW outBuf[],     INT4 *outPLen){ 
  ...
  const CHAR2 crOut= debug? CRVIS: CR;
...
#if USE_ALLOCA
  CHARW *codeBuf = alloca( codeMax * sizeof(CHARW) );  // Automatically freed...
#else 
  CHARW *codeBuf = malloc( codeMax * sizeof(CHARW) );  // Manually freed...
#endif
